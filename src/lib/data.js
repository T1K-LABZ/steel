// Static content for the landing page

export const HERO_IMAGE =
  "/images/hero-local.jpeg";

export const WORKSHOP_IMAGE =
  "/images/workshop.png";

export const SERVICES = [
  "Gates",
  "Balconies",
  "Stair Railings",
  "Windows",
  "Doors",
  "Stainless Steel Works",
  "Custom Fabrication",
  "Security Grills",
];

export const CATEGORIES = [
  "All",
];

export const PORTFOLIO = [
  {
    id: "gate-1",
    title: "Sliding Compound Gate",
    category: "Gates",
    image: "/images/portfolio-gate-01.jpeg",
    size: "lg",
  },
  {
    id: "gate-2",
    title: "Decorative Swing Gate",
    category: "Gates",
    image: "/images/portfolio-gate-02.jpeg",
  },
  {
    id: "gate-3",
    title: "Arched Security Gate",
    category: "Gates",
    image: "/images/portfolio-gate-03.jpeg",
  },
  {
    id: "gate-4",
    title: "Modern Slatted Gate",
    category: "Gates",
    image: "/images/portfolio-gate-04.jpeg",
  },
  {
    id: "gate-5",
    title: "Custom Estate Gate",
    category: "Gates",
    image: "/images/portfolio-gate-05.jpeg",
  },
  {
    id: "gate-6",
    title: "Steel Driveway Gate",
    category: "Gates",
    image: "/images/portfolio-gate-06.jpeg",
  },
  {
    id: "door-1",
    title: "Steel Security Door",
    category: "Doors",
    image: "/images/portfolio-door-01.jpeg",
  },
  {
    id: "door-2",
    title: "Decorative Metal Door",
    category: "Doors",
    image: "/images/portfolio-door-02.jpeg",
  },
  {
    id: "door-3",
    title: "Single Panel Door",
    category: "Doors",
    image: "/images/portfolio-door-03.jpeg",
  },
  {
    id: "door-6",
    title: "Double Steel Door",
    category: "Doors",
    image: "/images/portfolio-door-06.jpeg",
  },
  {
    id: "window-2",
    title: "Steel Window Grille",
    category: "Windows",
    image: "/images/portfolio-window-02.jpeg",
  },
];

export const FEATURES = [
  {
    icon: "shield-check",
    title: "Premium Materials",
    body: "We source mild steel, stainless steel and aluminium from trusted Kenyan and imported suppliers — built to last decades.",
  },
  {
    icon: "hammer",
    title: "Skilled Craftsmanship",
    body: "Over 12 years of welding, finishing and installation experience. Every joint, weld and paint coat is hand-inspected.",
  },
  {
    icon: "compass",
    title: "Modern Designs",
    body: "From minimalist contemporary to traditional Kenyan motifs — we collaborate with architects and homeowners to design the perfect piece.",
  },
  {
    icon: "calendar-clock",
    title: "Timely Delivery",
    body: "Clear timelines, weekly progress updates and on-time installation. No surprises, no excuses.",
  },
  {
    icon: "map-pin",
    title: "Nairobi-Based",
    body: "Workshop in Ruiru with site service across Juja, Nairobi, Kiambu, Thika and beyond.",
  },
  {
    icon: "wrench",
    title: "Custom Fabrication",
    body: "One-off, bespoke pieces tailored to your exact dimensions, finish and budget. No template work.",
  },
];

export const TESTIMONIALS = [
  {
    name: "Wanjiku Mwangi",
    role: "Homeowner, Karen",
    body: "Our new sliding gate is genuinely stunning. The finish is flawless and the team installed it in a single day. Worth every shilling.",
    rating: 5,
  },
  {
    name: "Eng. Brian Otieno",
    role: "Project Manager, Westlands",
    body: "We've used Shiloki Steel Solution on three commercial projects. Quality, lead times and communication are best-in-class in Nairobi.",
    rating: 5,
  },
  {
    name: "Aisha Hassan",
    role: "Architect, Kileleshwa",
    body: "Their attention to detail on the spiral staircase was incredible — every weld ground smooth, every screw flush. Highly recommend.",
    rating: 5,
  },
  {
    name: "David Kamau",
    role: "Developer, Ruaka",
    body: "Custom balcony railings for a 14-unit apartment block. Delivered ahead of schedule and within budget. Will be using them again.",
    rating: 5,
  },
];

export const STATS = [
  { value: "12+", label: "Years of craft" },
  { value: "480+", label: "Projects delivered" },
  { value: "98%", label: "On-time installation" },
  { value: "5★", label: "Average client rating" },
];
